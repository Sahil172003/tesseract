...
router = APIRouter(prefix="/api/openai-chat", tags=["openai-chat"])
...
class OpenAIChatManager:
    """OpenAI-based document chat manager with session handling"""
    ...
    def create_session(self, file_content: bytes, filename: str) -> str:
        ...
        try:
            ...
            # Generate session ID
            session_id = f"openai_session_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{len(self.sessions)}"
    ...
    def answer_question(self, session_id: Optional[str], question: str, model: str = "gpt-4o") -> str:
        """Answer question about document using OpenAI or general chat"""
        try:
            ...
            if session_id and session_id in self.sessions:
                ...
                if session['vector_store'] and self.embeddings:
                    try:
                        ...
                        context = session['content'][:16000]  # GPT-4 can handle more context
                else:
                    context = session['content'][:16000]  # Use first 16000 chars for GPT-4
...
Please provide a clear and informative answer based only on the information available in the document. If the information is not available in the document, please state that clearly."""
                ...
                prompt = f"""You are ChatGPT, a helpful AI assistant created by OpenAI. Please answer the following question in a clear, informative, and conversational way.
    ...
    def _get_fallback_response(self, session_id: Optional[str], question: str) -> str:
        ...
        if session_id and session_id in self.sessions:
            ...
            if any(word in question_lower for word in ['what', 'describe', 'explain']):
                ...
                return f"I can see numerical information in {session['filename']}, but I need OpenAI API access to provide accurate analysis."
                ...
                return f"I can see your question relates to {session['filename']}, but I need OpenAI API access to provide detailed analysis. Please ensure your OpenAI API key is configured."
        else:
            return "I can provide general assistance, but I need OpenAI API access to give you the best responses. Please ensure your OpenAI API key is configured."
...
openai_chat_manager = OpenAIChatManager()
...
async def upload_document(file: UploadFile = File(...)):
    """Upload and process document for OpenAI chat"""
    try:
        # Validate file format
        supported_formats = openai_chat_manager.processor.get_supported_formats()
        ...
        # Create session
        session_id = openai_chat_manager.create_session(file_content, file.filename)
        session = openai_chat_manager.get_session(session_id)
...
async def chat_with_document(request: ChatRequest):
    """Chat with uploaded document using OpenAI or general chat"""
    try:
        logger.info(f"Processing OpenAI chat question: {request.question}")
        
        # Answer question (works with or without session_id)
        answer = openai_chat_manager.answer_question(
            request.session_id, 
            request.question, 
            request.model
        )
        ...
        history = [
            ...
            for msg in openai_chat_manager.get_chat_history(request.session_id)
        ]
        
        # Get document info if session exists
        document_info = None
        if request.session_id:
            session = openai_chat_manager.get_session(request.session_id)
            if session:
                document_info = {
...
async def get_sessions():
    ...
    try:
        sessions = openai_chat_manager.get_all_sessions()
...
async def delete_session(session_id: str):
    """Delete a specific OpenAI session"""
    try:
        success = openai_chat_manager.clear_session(session_id)
        if success:
...
async def clear_general_chat():
    """Clear general OpenAI chat history"""
    try:
        openai_chat_manager.clear_general_chat()
...
async def test_endpoint():
    """Test endpoint to verify OpenAI API functionality"""
    return {
        "status": "success",
        "message": "OpenAI document chat API is running",
        "timestamp": datetime.now().isoformat(),
        "supported_formats": openai_chat_manager.processor.get_supported_formats(),
        "active_sessions": len(openai_chat_manager.sessions)
    }
...
async def health_check():
    ...
    try:
        return {
            ...
            "embeddings_available": openai_chat_manager.embeddings is not None,
