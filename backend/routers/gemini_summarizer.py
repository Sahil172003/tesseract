...
class GeminiChatManager:
    """Gemini-based document chat manager with session handling"""
    ...
    def create_session(self, file_content: bytes, filename: str) -> str:
        ...
        try:
            ...
            # Generate session ID
            session_id = f"gemini_session_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{len(self.sessions)}"
    ...
    def answer_question(self, session_id: Optional[str], question: str, model: str = "gemini-1.5-pro") -> str:
        """Answer question about document using Gemini or general chat"""
        try:
            ...
            if session_id and session_id in self.sessions:
                ...
                if session['vector_store'] and self.embeddings:
                    try:
                        ...
                        context = session['content'][:30000]  # Gemini can handle larger context
                else:
                    context = session['content'][:30000]  # Use first 30000 chars for Gemini
...
Please provide a clear and informative answer based only on the information available in the document. If the information is not available in the document, please state that clearly."""
                ...
                prompt = f"""You are Gemini, a helpful AI assistant created by Google. Please answer the following question in a clear, informative, and conversational way.
    ...
    def _get_fallback_response(self, session_id: Optional[str], question: str) -> str:
        ...
        if session_id and session_id in self.sessions:
            ...
            if any(word in question_lower for word in ['what', 'describe', 'explain']):
                ...
                return f"I can see numerical information in {session['filename']}, but I need Gemini API access to provide accurate analysis."
                ...
                return f"I can see your question relates to {session['filename']}, but I need Gemini API access to provide detailed analysis. Please ensure your Google API key is configured."
        else:
            return "I can provide general assistance, but I need Gemini API access to give you the best responses. Please ensure your Google API key is configured."
...
async def upload_document(file: UploadFile = File(...)):
    """Upload and process document for Gemini chat"""
    try:
        # Validate file format
        supported_formats = gemini_chat_manager.processor.get_supported_formats()
        ...
        # Create session
        session_id = gemini_chat_manager.create_session(file_content, file.filename)
        session = gemini_chat_manager.get_session(session_id)
...
async def chat_with_document(request: ChatRequest):
    """Chat with uploaded document using Gemini or general chat"""
    try:
        logger.info(f"Processing Gemini chat question: {request.question}")
        
        # Answer question (works with or without session_id)
        answer = gemini_chat_manager.answer_question(
            request.session_id, 
            request.question, 
            request.model
        )
        ...
        history = [
            ...
            for msg in gemini_chat_manager.get_chat_history(request.session_id)
        ]
        
        # Get document info if session exists
        document_info = None
        if request.session_id:
            session = gemini_chat_manager.get_session(request.session_id)
            if session:
                document_info = {
...
async def get_sessions():
    """Get all active Gemini sessions"""
    try:
        sessions = gemini_chat_manager.get_all_sessions()
...
async def delete_session(session_id: str):
    """Delete a specific Gemini session"""
    try:
        success = gemini_chat_manager.clear_session(session_id)
        if success:
...
async def clear_general_chat():
    """Clear general Gemini chat history"""
    try:
        gemini_chat_manager.clear_general_chat()
...
async def test_endpoint():
    """Test endpoint to verify Gemini API functionality"""
    return {
        "status": "success",
        "message": "Gemini document chat API is running",
        "timestamp": datetime.now().isoformat(),
        "supported_formats": gemini_chat_manager.processor.get_supported_formats(),
        "active_sessions": len(gemini_chat_manager.sessions)
    }
...
async def health_check():
    ...
    try:
        return {
            ...
            "embeddings_available": gemini_chat_manager.embeddings is not None,
