...
router = APIRouter(prefix="/api/document-chat", tags=["document-chat"])
...
class DocumentChatManager:
    """Enhanced document chat manager with session handling"""
...
chat_manager = DocumentChatManager()
...
async def upload_document(file: UploadFile = File(...)):
    """Upload and process document for chat"""
    try:
        # Validate file format
        supported_formats = chat_manager.processor.get_supported_formats()
        ...
        # Create session
        session_id = chat_manager.create_session(file_content, file.filename)
...
async def health_check():
    ...
    try:
        return {
            ...
            "embeddings_available": chat_manager.embeddings is not None
        }
    except Exception as e:
        return {
            "status": "unhealthy",
