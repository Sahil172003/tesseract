...
export default function OpenAIOCRChatPage() {
  ...
  const loadAvailableModels = async () => {
    try {
      const response = await fetch("http://localhost:8000/api/openai-chat/models")
      if (response.ok) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:8000/api/openai-chat/upload", {
        method: "POST",
        body: formData,
      })
      ...
      if (response.ok) {
        ...
        toast({
          ...
          description: `You can now chat with ${data.filename} using OpenAI.`,
          variant: "default",
        })
      } else {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await fetch("http://localhost:8000/api/openai-chat/chat", {
        method: "POST",
        headers: {
  ...
  const handleClearChat = async () => {
    ...
    if (!session) {
      try {
        await fetch("http://localhost:8000/api/openai-chat/chat/clear", {
          method: "DELETE",
        })
      } catch (error) {
  ...
  return (
    <div className="new">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        <h1 className="text-3xl font-bold tracking-tight gradient-text">OpenAI GPT Assistant</h1>
        <p className="text-muted-foreground mt-1">Chat with OpenAI GPT models or upload a document for intelligent document analysis and Q&A.</p>
      </motion.div>
            ...
            <CardContent className="p-4">
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-medium">OpenAI Model Selection</h3>
                  <button
                  ...
                  {showModelSelector && (
                    ...
                    >
                      ...
                      <p className="text-xs text-muted-foreground mt-2">
                        Choose the OpenAI model for your conversations. Works for both general chat and document analysis.
                      </p>
                    </motion.div>
                  )}
                </AnimatePresence>
                  ...
                  {isProcessing ? (
                    <div className="space-y-3 p-4">
                      ...
                      <p className="text-xs text-muted-foreground text-center">
                        Analyzing document content and preparing for OpenAI chat...
                      </p>
                    </div>
                  ) : (
                    <motion.div
                    ...
                    >
                      ...
                      <div className="text-center">
                        <p className="text-xs text-green-600 font-medium">
                          ✓ Document ready for OpenAI analysis
                        </p>
        ...
        >
          ...
          <div className="flex items-center justify-between p-4 border-b bg-background/50 backdrop-blur-sm">
            <div className="flex items-center space-x-2">
              ...
              <div>
                <h3 className="text-sm font-medium">OpenAI GPT</h3>
            ...
            {messages.length === 0 ? (
              <div className="flex h-full flex-col items-center justify-center text-center">
                ...
                <h3 className="text-lg font-medium">
                  {currentDocumentInfo ? "Chat with your document using OpenAI" : "Start a conversation with OpenAI GPT"}
                </h3>
                <p className="mt-2 text-sm text-muted-foreground max-w-md">
                  {currentDocumentInfo 
                    ? `Ask questions about ${currentDocumentInfo.filename}. OpenAI GPT will analyze the document and provide detailed, accurate answers.`
                    : "Ask me anything! You can have a general conversation with OpenAI GPT or upload a document for document-specific analysis."
                  }
                </p>
                {!currentDocumentInfo && (
                  <Button
                  ...
                  >
                    <div className="chat-bubble-ai">
                      <div className="flex items-center space-x-2">
                        ...
                        <p className="text-sm">
                          {currentDocumentInfo ? "OpenAI GPT is analyzing the document..." : "OpenAI GPT is thinking..."}
                        </p>
                      </div>
                    </div>
                  </motion.div>
          ...
          <div className="border-t p-4 bg-background/50 backdrop-blur-sm">
            <div className="flex space-x-2">
              <Textarea
                placeholder={
                  currentDocumentInfo
                    ? `Ask OpenAI GPT about ${currentDocumentInfo.filename}...`
                    : "Ask OpenAI GPT anything or upload a document for analysis..."
                }
                value={inputMessage}
